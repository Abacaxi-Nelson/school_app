...
class PhoneNumberField extends StatelessWidget {
  ...
  const PhoneNumberField({Key key, this.controller, this.prefix}) : super(key: key);
...
class SubTitle extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Align(
        alignment: Alignment.centerLeft, child: Text(' $text', style: TextStyle(color: Colors.white, fontSize: 14.0)));
  }
}
...
class ShowSelectedCountry extends StatelessWidget {
  ...
  const ShowSelectedCountry({Key key, this.onPressed, this.country}) : super(key: key);
...
class SelectableWidget extends StatelessWidget {
  ...
  const SelectableWidget({Key key, this.selectThisCountry, this.country}) : super(key: key);
